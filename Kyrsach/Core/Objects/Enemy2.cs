...
namespace Kyrsach.Core.Objects
{
    ...
    {
        public Enemy2(Properties props) : base(props)
        {
        ...
        public override bool Update(float dt)
        {
            ...
            if (player.Length() <= 800 && CurrentLevel.GetInstance().GetNumberLevel() != 2)
            {
            ...
            if (Target_Player == true)
            {
                ...
                for (int i = 0; i < AllCharacter.GetInstance().GetPlayers().Count; i++)
                {
                    if (CollisionHandler.CollisionHandler.GetInstance().CheckCollision(m_Collider.Get(), AllCharacter.GetInstance().GetPlayers()[i].GetCollider().Get()))
                    {
                        m_Transform.X = m_LastSafePosition.X;
                        m_Count = 0;
                    }
